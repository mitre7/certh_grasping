...
//    cout << "Drop position: " << drop_position << endl;
    ...
    for (uint i=0; i<cg.spring_list.size(); i++)
    {
        ...
        cout << endl << "Spring World Coordinates: " << endl << spring_position_world << endl;
...
//        cout << "Gripper angle with tranformations: " << gripper_angle_2 * 180 / M_PI << endl;
        ...
        gripper_position = cg.calculateGripperPosition(spring_position_world, gripper_angle_2, cg.is_cluttered[i]);

        if(cg.is_cluttered[i])
            cout << "Spring over debris" << endl;
        else
...
//        if (!arm.openGripper()) continue;
        if (!gripper.changeGripperState(cg.gripper_opening_angle)) continue;
